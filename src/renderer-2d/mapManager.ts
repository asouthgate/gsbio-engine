...
export class MapManager {
  ...
  addResultLayer(runId: string, layerId: string, envelope: any) {
    ...
    if (envelope.kind === 'geojson') {
      ...
      this.map.addLayer({ id: `${srcId}-fill`, type: 'fill', source: srcId, filter: ['==', ['geometry-type'], 'Polygon'], paint: { 'fill-color': rp.fillColor, 'fill-opacity': rp.fillOpacity } });
      this.map.addLayer({ id: `${srcId}-line`, type: 'line', source: srcId, filter: ['==', ['geometry-type'], 'LineString'], paint: { 'line-color': rp.lineColor, 'line-width': rp.lineWidth } });
      this.map.addLayer({ id: `${srcId}-circle`, type: 'circle', source: srcId, filter: ['==', ['geometry-type'], 'Point'], paint: { 'circle-radius': rp.circleRadius, 'circle-color': rp.circleColor } });
      ...
      this.map.addLayer({ id: `${srcId}-raster`, type: 'raster', source: srcId });
    } else {
      ...
      if (envelope.type === 'raster') {
        ...
        this.map.addLayer({ id: `${srcId}-line`, type: 'line', source: srcId, 'source-layer': envelope.sourceLayer, paint: { 'line-color': rp.lineColor, 'line-width': rp.lineWidth } });
        this.map.addLayer({ id: `${srcId}-fill`, type: 'fill', source: srcId, 'source-layer': envelope.sourceLayer, paint: { 'fill-color': rp.fillColor, 'fill-opacity': rp.fillOpacity } });
      }
    }
