...
3. **Network is owned by providers, not by the view.** React components never see URLs, API keys, or binary parsing; they ask the engine for state and the engine asks `@gsbio/client` for raw data.

## Layer responsibilities

| Layer | Responsibility | Package |
| --- | --- | --- |
| Data source / storage | Manage raw GIS / map data, feature attributes, spatial indexing. | `@gsbio/core` (registries) + `@gsbio/client` (fetching) |
| Analysis plugin | Take inputs, perform math, return results. | `@gsbio/core` (`ModelDef` + `Executor`) |
| Engine core | Coordinate events, provide the projection service, own the state tree. | `@gsbio/core` (`SimulationEngine`) |
| View (React / Canvas) | Listen for state changes and paint. | `@gsbio/react` + `@gsbio/react-ui` + a renderer plugin |
...
`CoordinateService` (in `@gsbio/core`) exposes projection functions both sides share. The analysis side uses them for spatial queries (`pointInPolygon`); the render side uses them for drawing (`lngLatToPixel`). The projection function pair is supplied by the renderer — maplibre provides `map.project` / `map.unproject`, a WebGPU renderer would supply its own — so the engine stays free of any map library.
...
Provides `EngineProvider`, `AppProvider` (composite), `DataSourceProvider`, and hooks `useEngine`, `useEngineState`, `useDraw`, `useModel`, `useRun`, `useResults`, `useDataSources`. The package `peerDepends` on `react` / `react-dom` and `@gsbio/core`; no dependency on `maplibre-gl`, `terra-draw`, or any WebGPU library.
...
1. depends on `@gsbio/core` for types (and optionally the `SimulationEngine` class),
...
`@gsbio/renderer-2d` — MapLibre + TerraDraw. `@gsbio/renderer-3d` — WebGPU stub (its `mount()` rejects; a placeholder so the demo's error handling is exercisable).

Renderers are separate packages so consumers pay for what they use (a 2D-only consumer doesn't install WebGPU/WASM toolchain) and so renderer choice is a runtime concern (swap via the `<Canvas renderer={...}>` prop without touching anything else). Neither renderer imports from `@gsbio/react`.
...
If `core` imported `fetch` it would no longer be usable in pure-Node contexts. Keeping the engine headless means it has **no URL strings, no API keys, no parsers** anywhere in its source tree. Boundary rule: `@gsbio/client` may import from `@gsbio/core`; never the reverse.
...
- `@gsbio/core` imports nothing from React, maplibre, terra-draw, or the network. If a future change needs any of those, the change belongs in another package — not in `core`.