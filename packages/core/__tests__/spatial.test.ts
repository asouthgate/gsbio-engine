...
describe('pointInCircle', () => {
  it('true inside, false outside', () => {
    expect(pointInCircle(destinationPoint(EXETER, 100, 0), EXETER, 200)).toBe(true);
    expect(pointInCircle(destinationPoint(EXETER, 300, 0), EXETER, 200)).toBe(false);
  });
  it('true at the exact radius within rounding tolerance', () => {
    const edge = destinationPoint(EXETER, 200, 45);
    // Floating-point round-trip (destinationPoint → haversine) accumulates
    // sub-microscopic error; treat within 1 cm as on the boundary.
    expect(pointInCircle(edge, EXETER, 200 + 0.01)).toBe(true);
  });
});
