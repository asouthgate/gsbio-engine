/**
 *demo-web demo provider — the canonical "this is what a dev writes" example
 * of consuming the catshark engine.
 *
 * What the dev owns here, and nothing else:
 *
 *   1.  `customRadiusRasterModel`           — the model *schema* (id, params).
 *   2.  `customRadiusRasterProvider`        — a `ComputeProvider`, i.e. the
 *                                            `preprocess` + `submit` blanks:
 *                                            preprocess picks which drawn
 *                                            features go in; submit hands the
 *                                            payload to a `StubExecutor` and
 *                                            supplies the pure rasterisation
 *                                            kernel.
 *   3.  `installCustomRadiusRaster(engine)` — registers the model + provider
 *                                            once and selects the model.
 *
 * What the dev *does not* write: run triggering, cancellation, progress
 * plumbing, run history, result lifecycle (show/hide/clear, per-layer
 * toggle), map-layer fan-out. All of it is the engine + renderer's job; this
 * provider just describes the data flow and the math.
 */
import {
  createStubExecutor,
  registerModel,
...
/* ---------------------------- Preprocess blank --------------------------- */
//
// Pick which drawn features reach `submit`. The dev's policy lives here: the
// engine hands in *every* drawn feature and lets each model decide what to
// keep. Here we forward only the `Custom_radius`-labelled circles' geometric
// truth (`center` + `radiusMeters`), exactly as the engine stores them on
// `DrawnFeature.circle`.
...
/* ------------------------------ Submit blank ----------------------------- */
//
// Simulate a backend roundtrip via `StubExecutor` (engine-supplied, headless
// harness: queue latency + streaming progress ticks + cancellation). The dev
// supplies only the *kernel*: a pure function from preprocessed payload to
// `RunResult`. Here the kernel rasterises each circle into a 64x64 image
// shaded by normalised distance from the centre, alpha-masked to the circle,
// and returns one `{ kind: 'image', url, bounds }` envelope per circle.
//
// The per-layer id is the **source circle's drawn-feature id**, so each
// raster is explicitly associated with the circle that produced it — when
// shown, it appears "under" that circle on the map.
...
/* ------------------------------- Wiring --------------------------------- */
//
// Register the model + provider once and select the model as current. Call
// on app boot, right after creating the engine but before the first `run()`.
