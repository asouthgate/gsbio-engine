...
export function ModelPanel() {
  const { state, dispatch, run } = useModel();
  const { state: drawState } = useDraw();
    ...
    <div className="panel-section">
      ...
      {def && (
        <button
          ...
          onClick={() => run(drawState.features)}
          disabled={state.isRunning}
        >
          {state.isRunning ? 'Running…' : 'Run model'}
        </button>
      )}

      {state.lastRunAt != null && (
        <p className="hint">Last run at {new Date(state.lastRunAt).toLocaleTimeString()}.</p>
      )}
    </div>
  );
}