import { StrictMode } from 'react';
import { createRoot } from 'react-dom/client';
import { createSimulationEngine } from '@gsbio/core';
import { AppProvider } from '@gsbio/react';
import { App } from './components/App';
import { installRadialSpread } from './models/radialSpread/model';
import { installForageCost } from './models/forageCost/model';
import { installCorridorConnect } from './models/corridorConnect/model';
import './styles/index.css';

// Construct a single engine instance, install the three demo models + their
// executors, and select one — then hand the engine to <AppProvider>. Apps
// pass an explicit `engine` to <AppProvider> whenever they need to wire
// executors (the default path constructs an empty engine with only the
// built-ins).
const engine = createSimulationEngine();
installRadialSpread(engine);
installForageCost(engine);
installCorridorConnect(engine);
// Demo UX: render a freshly-succeeded run's layers on the map immediately so
// the user doesn't have to expand the Results section + click "Show on map".
engine.autoShowResults = true;

createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <AppProvider engine={engine}>
      <App />
    </AppProvider>
  </StrictMode>,
);